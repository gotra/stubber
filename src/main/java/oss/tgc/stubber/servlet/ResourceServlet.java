...
public class ResourceServlet extends HttpServlet {
    ...
    protected LookupResult lookupNoCache(HttpServletRequest req) {
        ...
        final URL url;
        url = getServletContext().getClassLoader().getResource(path);
        if(url == null)
...
//                        return new Error(HttpServletResponse.SC_FORBIDDEN, "Forbidden");
//                    else
                        return new StaticFile(con.getLastModified(),mimeType,(int)con.getContentLength(),acceptsDeflate(req),url);
                } else
                    // Unexpected?
